...
## 2026-03-11 S2-5 콘텐츠 허브 기획 메모

- 새 개별페이지 `콘텐츠허브` 는 목록/상세의 보조 링크가 아니라 수강생 설득용 상단 퍼널로 본다.
- 허브는 `전국 클래스 탐색`, `파트너 스토리`, `계절 트렌드`, `초보자 가이드` 를 한 화면에서 이어줘야 한다.
- 1차는 전용 CMS 없이 시작하고, `class-api?action=getContentHub` 응답 하나로 페이지를 구성하는 방식이 가장 빠르다.
- 실배포 전까지는 새 page id 만 미정 상태이며, 링크 목적지는 `2606/2607/2609` 로 우선 고정한다.
