...
## 고객 여정 맵핑 프레임워크 (AIDA → Retention)

### AIDA 퍼널 단계별 설계

```
[A] Awareness (인지)
  → 메이크샵 메인 인기 클래스 카드 (WF-01 연동)
  → SEO 클래스 상세 페이지 Schema.org 마크업
  → 소셜 공유 기능 (카카오/URL 복사)
  → 전환 지표: 목록 페이지 UV (목표 월 300+)

[I] Interest (흥미)
  → 목록 페이지 카드 (카테고리/가격/난이도 필터)
  → 찜하기 기능 (localStorage, 최근 본 클래스)
  → 전환 지표: 상세 페이지 CTR (목표 35%+)

[D] Decision (결정)
  → 상세 페이지 (커리큘럼/재료/강사 정보/후기)
  → 라이트박스 이미지 확대
  → 관련 클래스 추천
  → 전환 지표: 예약 클릭률 (목표 15%+)

[A] Action (행동)
  → 메이크샵 네이티브 결제 (basket.action.html POST)
  → WF-04 예약 기록 → WF-05 정산 생성
  → 수강 확인 이메일
  → 전환 지표: 결제 완료율 (목표 70%+)

[R] Retention (재방문)
  → D-3/D-1 리마인더 이메일 (WF-11)
  → 수강 완료 후기 요청 (WF-12 +7일)
  → 파트너 답글 → 후기 품질 향상
  → 전환 지표: 재구매율 (목표 20%+)
```
...
| 단계 | 이탈률 기준치 | 개선 액션 |
|------|------------|---------|
| 목록→상세 | 65% 이탈 허용 (35% CTR 목표) | 카드 썸네일/가격 개선 |
| 상세→예약 클릭 | 85% 이탈 허용 (15% 클릭 목표) | CTA 위치/문구 최적화 |
| 예약→결제 완료 | 30% 이탈 허용 (70% 완료 목표) | 결제 흐름 단순화 |

### CTA 배치 전략

```
PC 상세 페이지:
  - 상단: "지금 예약하기" (히어로 영역 우측 스티키 박스)
  - 중단: 일정 선택 후 "예약하기" 인라인 버튼
  - 하단: 섹션 끝 CTA 배너

모바일 상세 페이지:
  - 하단 고정 바: 가격 + "예약하기" (항상 노출)
  - 스크롤 중 히어로 영역 벗어나면 상단 미니 CTA 노출
```
...
```javascript
// 핵심 이벤트 (구현 우선순위 순)
gtag('event', 'view_class_list', { category: 'catalog', filter: 'all' });
gtag('event', 'view_class_detail', { class_id: 'CL_202602_001', class_name: '...' });
gtag('event', 'click_booking_btn', { class_id: '...', price: 50000 });
gtag('event', 'complete_purchase', { value: 50000, currency: 'KRW' });
gtag('event', 'apply_partner', { specialty: '압화' });
gtag('event', 'complete_education', { score: 90 });
```
