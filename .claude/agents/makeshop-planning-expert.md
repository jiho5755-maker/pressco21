...
**가상태그 핵심 개념:**
- 형식: `<!--/태그명/-->` (HTML 주석 형식, 메이크샵 서버가 값으로 치환)
- `{$member_id}` 같은 형식은 메이크샵에서 사용하지 않음 (이전 잘못된 정보)
- 서버 사이드 렌더링: HTML이 브라우저에 도달하기 전에 가상태그가 실제 값으로 대체
- IF 조건 가상태그 닫기: 무조건 `<!--/end_if/-->` 하나만 사용
- **개별 페이지(page.html)에서 가상태그 동작 여부: 테스트 중 (v2)**

**회원 관련 가상태그 전체 목록 (공식 문서 + 마이페이지 메인, 2026-02-21):**

| 가상태그 | 용도 | 비고 |
|---------|------|------|
| `<!--/user_id/-->` | 회원 아이디 | **파트너 인증 핵심** |
| `<!--/user_name/-->` | 회원 이름 | |
| `<!--/user_email/-->` | 회원 이메일 | |
| `<!--/user_mobile/-->` | 회원 휴대전화번호 | |
| `<!--/user_tel/-->` | 회원 연락처 | |
| `<!--/user_nickname/-->` | 회원 닉네임 | |
| `<!--/user_address/-->` | 회원 주소 | |
| `<!--/user_reserve/-->` | 회원 적립금 | |
| `<!--/user_deposit/-->` | 회원 예치금 | |
| `<!--/user_greeting/-->` | 회원 인사말 | |
| `<!--/user_basket_quantity/-->` | 장바구니 수량 | |
| `<!--/img_user/-->` | 회원 이미지 | |
| `<!--/join_date/-->` | 회원 가입일 | |
| `<!--/group_name/-->` | 회원 그룹 | |
| `<!--/group_level/-->` | 회원 그룹 등급 | |
| `<!--/group_icon/-->` | 회원 그룹 아이콘 | |
| `<!--/group_image/-->` | 회원 그룹 이미지 | |
| `<!--/group_reserve_message/-->` | 적립금 설정 문구 | |
| `<!--/attendance_count/-->` | 출석 체크 참여수 | 회원만 |
| `<!--/if_login/-->...<!--/end_if/-->` | 로그인 시에만 표시 | |
| `<!--/if_company_user/-->...<!--/end_if/-->` | 기업회원 여부 | IF 조건문 |
| `<!--/if_new_join_type/-->...<!--/end_if/-->` | 통합템플릿 사용여부 | IF 조건문 |
| `<!--/link_join/-->` | 회원 가입/정보 수정 링크 | |
| `<!--/link_withdraw/-->` | 회원 탈퇴 링크 | |

> **주의**: 위 태그들은 "마이페이지 메인"에서 확인된 목록. 개별 페이지(page.html)에서의 동작 여부는 v2 테스트로 확인 중.

**파트너 인증 아키텍처 (가상태그 동작 확인 후 확정):**
```
[HTML] <!--/user_id/--> -> 서버가 "jihoo5755"로 렌더링
  |
[JS] var userId = document.getElementById('ms-uid').textContent.trim();
  |
[GAS 요청] fetch(GAS_URL + '?member_id=' + encodeURIComponent(userId))
  |
...
**치환코드 (Task 151): -- 실배포 테스트 진행 중 (2026-02-21)**
- [x] `{$member_id}` 비로그인 시 출력값 확인 -> 개별페이지에서 **미치환(문자열 그대로)** 확인. 로그인 테스트 필요
- [ ] `{$member_id}` 로그인 시 정확한 회원 ID 출력 확인 -> **로그인 상태 테스트 필요**
- [ ] HTML 내 치환코드 삽입 -> JS에서 DOM으로 읽기 가능 여부 -> 로그인 테스트 후 확인
- [x] 상품 옵션 20/50/100개 등록 시 제한 여부 -> **제한 없음** (관리자 직접 테스트 확인)
- [x] 옵션별 재고(정원) 개별 설정 가능 여부 -> **가능** (관리자 직접 테스트 확인)

**IF 가상태그 올바른 형식 (2026-02-21 확인):**
- 열기: `<!--/if_login/-->` | 닫기: `<!--/end_if/-->` (모든 IF 태그 동일)
- 틀린 형식: `<!--/endif_login/-->`, `<!--/end_if_login/-->` 모두 오류 발생
- 개별페이지에서 `<!--/if_login/-->` 동작 확인됨
