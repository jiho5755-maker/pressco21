...
> **플랫폼**: 메이크샵 D4 프론트엔드 + n8n 자동화 + Airtable 데이터
> **상태**: 설계 완료 — 구현 대기
...
Phase 2 개발은 Google Apps Script(GAS) 기반으로 5,271줄 완성되었으나, 운영 전 n8n + Airtable 전환을 결정했다. 핵심 이유는 다음과 같다.

| 항목 | GAS 한계 | n8n + Airtable 해결 |
|------|---------|-------------------|
...
| 확장성 | Sheets 10,000행 성능 저하 | Airtable 50,000행/테이블 |
| 텔레그램 알림 | 이메일만 가능 | @Pressco21_makeshop_bot 즉시 연동 |
| 캐싱 | GAS CacheService (최대 6시간) | 불필요 (Airtable 직접 조회 200~500ms) |
...
[n8n 자동화 레이어]
  ...
  CORS: Reverse Proxy (nginx) 설정
        |
        | Airtable API / 메이크샵 Open API
        |
[데이터 레이어]
  Airtable (app6MBsHo7AKwh5XD)
  8개 테이블, Linked Records, Rollup 자동 집계
        |
[메이크샵 네이티브]
...
[관리자]
  └─ 텔레그램 @Pressco21_makeshop_bot 실시간 알림 수신
       └─ 파트너 신청 → WF-08 승인 (Bearer Token)
            └─ Airtable 파트너 상세 생성 + 메이크샵 회원그룹 변경
...
Airtable Rollup 기능으로 파트너 상세에서 "총 정산 건수", "총 수수료", "평균 평점"을 자동 집계한다. GAS에서 수동 계산하던 집계를 Airtable이 자동 관리한다.
...
[ STEP 2 ] 관리자 검토 (1~3 영업일)
    → Airtable tbl_Applications 심사 대기 뷰 확인
    → 포트폴리오 URL, 인스타그램 확인 (갤러리 뷰 활용)

[ STEP 3 ] 승인/거절
    → 승인: WF-08 (Bearer Token) → 파트너 코드 PC_YYYYMM_NNN 발급
             → Airtable tbl_Partners 생성 (SILVER 등급)
             → 메이크샵 회원그룹 변경 (파트너 그룹)
...
[ STEP 5 ] 클래스 등록 및 활동 시작
    ...
    → 관리자 검수 (1~2 영업일) 후 Airtable tbl_Classes 생성
    → 메이크샵에 상품 등록 (결제 처리용)
...
```
결제 감지 (WF-05, 10분 폴링)
  → Airtable 정산 내역 생성 (status: PENDING_SETTLEMENT)
...
```
Airtable 조회: status = FAILED AND retry_count < 5
...
```
Airtable status = COMPLETED 정산 내역 조회
  → 파트너별 누적 적립금 집계
...
[Phase A] 인프라 준비 (1~2일)
  ...
  3. Airtable 테이블 8개 생성 + 필드 정의
  4. n8n Credentials 등록 (Airtable, Makeshop, Gmail, Telegram, Admin)
...
[Phase E] 데이터 마이그레이션 + 전환 (1~2일)
  12. Google Sheets → Airtable 기존 데이터 CSV import
...
|-----|---------|---------|---------|
| 신규 파트너 등록 | 5명 | 10명 | Airtable tbl_Partners 행 수 |
...
| 파트너 대시보드 응답 | 1초 이내 (Airtable 직접 조회 200~500ms) |
| 정합성 검증 불일치 | 0건/월 목표 |
...
WF-04 Record Booking에서 `bookingData.member_id === partner.member_id` 확인 후 정산 스킵 + 텔레그램 알림. Airtable에 `SELF_PURCHASE` 상태로 기록하여 추적 가능.
...
| 단계 | 방법 |
|------|------|
| 사전 검수 | Airtable tbl_Classes 등록 시 관리자 검수 (갤러리 뷰 활용) |
...
| DB | 무료 (Google Sheets) | Airtable 무료 (1,200행/Base) 또는 Team $20/월 (50,000행) |
| 이메일 | 무료 (Gmail 100건) | 무료 (Gmail SMTP 500건) |
| 알림 | 이메일만 | 무료 (텔레그램, 기존 봇 운영 중) |
| 월 합계 | $0 | $0 ~ $20 |

Airtable 무료 플랜(1,200행/Base) 초기 운영 가능. 정산 내역 1,200행 초과 시 Team 플랜 전환 또는 월별 아카이빙. 대안: n8n PostgreSQL 직접 사용 (비용 $0, GUI 관리 불가).

## 부록 B: 기존 GAS 코드와의 관계

- `파트너클래스/class-platform-gas.gs` (5,271줄): 마이그레이션 완료 후 1개월 이상 보존 (롤백 보험)
- GAS 코드의 수수료 로직, 마스킹 함수, 이메일 HTML 템플릿: n8n Code 노드로 이식
- Airtable 필드명: 기존 Sheets 컬럼명과 동일하게 유지하여 변환 로직 최소화
...
| n8n 아키텍처 설계 | `/docs/phase2/n8n-airtable-migration-architecture.md` | 13개 워크플로우 상세 + Airtable 8개 테이블 |
