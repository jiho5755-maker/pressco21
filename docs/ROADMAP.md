...
- **Task 201: Airtable 데이터베이스 설계 + 기초 데이터 입력** - 우선순위
...
- **Task 202: n8n 서버 환경 설정 (CORS, Credentials, Gmail SMTP)** - 우선순위
...
- **Task 223: WF-13 등급 자동 업데이트**
  ...
  - 구현 사항:
    - WF-13 Grade Update: Schedule 매일 06:00 -> tbl_Partners 전체 (status=active) -> 파트너별 tbl_Settlements (COMPLETED) 조회 -> 완료 건수 + 평균 평점 계산
    - 수정된 등급 기준 (business-model-optimization.md 반영):
      - GOLD: 완료 8건 + 평점 3.8 (기존 10건/4.0에서 완화)
      - PLATINUM: 완료 30건 + 평점 4.2 (기존 50건/4.5에서 완화)
    - 강등 없음: gradeOrder 비교로 상위 등급만 변경 (기존 GAS 정책 유지)
    - 등급 변경 시: tbl_Partners Update (grade, commission_rate, reserve_rate) + 승급 축하 이메일 + 텔레그램 알림
    - 배치 완료 시: 텔레그램 "등급 업데이트 완료: 변경 {N}건"
  - 테스트 체크리스트:
    - SILVER -> GOLD 전환: 8건 COMPLETED + 평점 3.8 이상인 파트너 등급 변경 확인
    - 강등 방지: GOLD 파트너의 건수가 줄어도 SILVER로 돌아가지 않는지 확인
    - 등급 변경 시 commission_rate, reserve_rate 자동 업데이트 확인
