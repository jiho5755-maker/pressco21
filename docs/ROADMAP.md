...
> **상태**: 진행 중 (Task 150~151 완료, Task 152 대기)
> **기간**: 1~2주
...
- **Task 151: 메이크샵 치환코드 + 옵션 제한 검증** - ✅ 완료 (2026-02-20)
  - See: `/tasks/151-makeshop-substitution-options.md`
  - 대상: 테스트 HTML 페이지, 의사결정 문서
  - 의존성: 없음
  - **결과: 치환코드 3단계 인증 확정, 옵션 50개 이하 안전 설계**
  - `{$member_id}`: 비로그인 시 빈 문자열, 로그인 시 실제 ID (Playwright 실측)
  - `<!--/if_login/-->`: 간편구매에서 이미 동작 중 (실증 증거)
  - JS -> GAS 전달 패턴 확립, Phase 2 파트너 인증 아키텍처 확정
  - 옵션 제한: 공식 문서 미명시, 안전 기준 50개 이하 권장
  - 상세: `docs/api-verification/substitution-options-result.md`
  - 실배포 테스트 가이드 포함 (추후 필요 시 활용)
  - 규모: S
...
- **Task 152: API 호출 예산(Budget) 산정 및 캐싱 전략 수립**
  ...
  - 구현 사항:
    ...
    - 데이터 유형별 캐싱 전략: 정적(24h), 반정적(1h), 실시간(캐싱 불가)
    - GAS 주문 폴링 간격 확정 (10~15분, 시간당 4~6회)
    - 일일 이메일 발송 카운트 모니터링 설계
...
| C-2 | 메이크샵 조회 API 500회/시간 경합 | 설계 완료, 실측 대기 | Task 152, Task 241 |
...
| 2026-02-20 | 1.3 | Task 151 완료 -- 치환코드 검증(빈 문자열 확인), Phase 2 파트너 인증 3단계 아키텍처 확정, 옵션 50개 이하 안전 설계 |
