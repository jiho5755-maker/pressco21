...
- **Task 151: 메이크샵 치환코드 + 옵션 제한 검증**
  ...
  - 구현 사항:
    ...
    - 파트너 인증 메커니즘 설계: 치환코드 -> JS -> GAS 파라미터 전달 경로 검증
    - 결과에 따른 Phase 2 아키텍처 의사결정 문서 작성
  - 테스트 체크리스트:
    - 비로그인/로그인 상태에서 치환코드 동작 확인
    - 상품 옵션 20개, 50개, 100개 등록 시 제한 여부
    - 치환코드가 JS에서 정상적으로 읽히는지 실제 페이지에서 검증
