...
| `gas-backend-expert` | ~~GAS~~ **n8n 워크플로우**, Airtable 설계, 이메일 자동화, 정산 | opus | 1.5, 2, 3 |
...
| **Phase 2 v2.0 (n8n + Airtable)** | | |
| 201 Airtable DB 설계 | `gas-backend-expert` | `ecommerce-business-expert`, `makeshop-planning-expert` |
...
> **상태**: n8n + Airtable 방식으로 재설계 진행 중 (GAS 코드 참고용 보존)
> **기간**: 2~3개월
> **시작**: Phase 1.5 API 검증 완료 후
> **목표**: 제휴업체 강의 홍보 + 결제 + 수수료 적립금 선순환 생태계 구축
> **규모**: XL
> **핵심 리스크**: 파트너 인증 메커니즘(C-1), API 호출 경합(C-2), Airtable 정산 무결성(C-3)
> **기술 전환**: GAS 5,271줄 -> n8n 13개 워크플로우 + Airtable 8개 테이블
...
- **Task 201: Airtable 데이터베이스 설계 + 기초 데이터 입력** - ✅ 설계 가이드 완료 (관리자 Airtable 설정 작업 필요)
  - See: `/tasks/201-airtable-database-design.md`
  - 대상: Airtable Base (app6MBsHo7AKwh5XD) 내 8개 테이블 신규 생성
  - 의존성: 없음 (Phase 1.5 검증 결과 참조)
  - 규모: M
  - 에이전트: `주도` gas-backend-expert | `협업` ecommerce-business-expert, makeshop-planning-expert
  - 구현 사항:
    - Airtable 테이블 8개 생성: tbl_Partners, tbl_Classes, tbl_Settlements, tbl_Applications, tbl_Reviews, tbl_PollLogs, tbl_EmailLogs, tbl_Settings
    - 각 테이블 필드 타입 설정 (Single Select, Link, Currency, Rating 등 Airtable 네이티브 타입 활용)
    - Linked Record 관계 설정 (Partners -> Classes -> Settlements -> Reviews)
    - Rollup 필드 설정 (파트너별 총 정산 건수, 평균 평점 자동 계산)
    - 뷰 설계: "활성 파트너", "대기 중 정산", "실패 정산", "심사 대기 신청" 등 운영 뷰 생성
    - tbl_Settings 초기 데이터 입력 (last_poll_time, class_category_id, admin_email)
    - GAS Sheets 기존 데이터 CSV export -> Airtable import 테스트 (본격 마이그레이션은 Phase 2-F)
  - 테스트 체크리스트:
    - 모든 테이블 필드 타입이 설계서(n8n-airtable-migration-architecture.md 3장)와 일치하는지 확인
    - Linked Record 관계가 올바르게 동작하는지 (Partners -> Classes 링크 테스트)
    - Rollup 필드가 자동 계산되는지 (예: 파트너에서 "총 정산 건수" 집계)
...
- **Task 202: n8n 서버 환경 설정 (CORS, Credentials, Gmail SMTP)** - ✅ 설정 가이드 완료 (관리자 서버 설정 작업 필요)
  ...
  - 구현 사항:
    ...
    - n8n Credentials 5개 등록: Airtable API Key, Makeshop API (Shopkey+Licensekey), Gmail SMTP (앱 비밀번호), Telegram Bot API Key, Admin Token
...
- **Task 211: WF-01~03 클래스 조회 + 파트너 인증 API 워크플로우** - 우선순위
  ...
  - 의존성: Task 201 (Airtable 테이블), Task 202 (n8n Credentials)
  - 규모: L
  - 에이전트: `주도` gas-backend-expert | `협업` makeshop-planning-expert, makeshop-ui-ux-expert
  - 구현 사항:
    - WF-01 Class API: Webhook GET -> Switch(action) -> Airtable 조회 -> filterByFormula 서버 필터링 -> JSON 응답
      - getClasses: 카테고리/난이도/유형/지역/가격대 필터 + 정렬 + 페이지네이션 (Airtable offset 기반)
      - getClassDetail: class_id 단일 조회 + 파트너 정보 Linked Record 조인
      - getCategories: 활성 클래스에서 카테고리 추출 + 건수 집계
    - WF-02 Partner Auth API: Webhook GET -> Switch(action) -> Airtable 파트너/신청 조회 -> 상태 분기
    ...
    - WF-03 Partner Data API: Webhook GET -> 파트너 인증 -> Airtable 조회 -> 마스킹
...
- **Task 212: WF-04 예약 기록 + 수수료 계산 + 정산 파이프라인** - 우선순위
  ...
  - 테스트 체크리스트:
    - 예약 기록 API 호출 -> Airtable 정산 내역에 PENDING_SETTLEMENT 상태로 기록 확인
...
- **Task 213: WF-05 주문 폴링 + 야간 배치 (동기화/정합성/재시도/D+3 정산)** - 우선순위
  ...
  - 구현 사항:
    ...
    - 5b. 상품 동기화 (매일 03:00): 메이크샵 상품 목록 -> Airtable 클래스 메타 비교 -> 신규/수정/비활성 처리
...
- **Task 221: WF-07~08 파트너 신청/승인 + 텔레그램 알림**
  ...
  - 의존성: Task 201 (Airtable tbl_Applications, tbl_Partners)
  - 규모: M
  - 에이전트: `주도` gas-backend-expert | `협업` brand-planning-expert, ecommerce-business-expert
  - 구현 사항:
    - WF-07 Partner Apply: Webhook POST -> 입력값 검증 + URL sanitize -> 중복 체크 (이미 파트너?/이미 심사 중?) -> Airtable tbl_Applications Create -> 접수 확인 이메일 -> 텔레그램 알림
      - application_id 생성: APP_YYYYMMDD_XXXXXX 형식
      - 텔레그램: "새 파트너 신청: {이름} ({공방명})"
    - WF-08 Partner Approve: Webhook POST -> 관리자 토큰 검증 (ADMIN_API_TOKEN) -> PENDING 확인 -> partner_code 생성 (PC_YYYYMM_NNN) -> tbl_Applications Update -> tbl_Partners Create (SILVER) -> 메이크샵 회원등급 변경 API -> 승인 이메일 -> 텔레그램 알림
      - 기존 FA-002 워크플로우 패턴(메이크샵 회원등급 변경) 재활용
    - 이메일 2종: 신청 접수 확인 + 승인 안내 (기존 GAS 이메일 카피 이식)
  - 테스트 체크리스트:
    - 파트너 신청 API -> Airtable tbl_Applications에 PENDING 상태 레코드 생성 확인
    - 중복 신청 방지: 이미 파트너/이미 심사 중일 때 적절한 에러 응답
    - 승인 처리 -> tbl_Partners에 SILVER 등급 레코드 생성 + partner_code 부여 확인
...
- **Task 222: WF-06, WF-09~10 클래스 관리 + 후기 답변 + 교육 인증**
  ...
  - 의존성: Task 211 (파트너 인증 API)
  - 규모: M
  - 에이전트: `주도` gas-backend-expert | `협업` brand-planning-expert
  - 구현 사항:
...
- **Task 231: WF-11~12 리마인더 + 후기 요청 이메일 자동화**
  ...
  - 구현 사항:
    ...
    - Gmail SMTP 일일 한도(500건) 미도달 시에도 tbl_EmailLogs에 발송 이력 기록
  - 테스트 체크리스트:
...
- **Task 241: 프론트엔드 URL 교체 (3줄) + 메이크샵 3페이지 등록**
  ...
  - 규모: S
  - 에이전트: `주도` makeshop-ui-ux-expert | `협업` gas-backend-expert, makeshop-planning-expert
  - 구현 사항:
...
- **Task 251: 전체 E2E 통합 테스트 + Airtable 관리자 워크플로우 검증**
  - See: `/tasks/251-n8n-e2e-integration-test.md`
  - 대상: 전체 Phase 2 v2.0 결과물 (n8n 13개 워크플로우 + Airtable 8개 테이블 + 프론트엔드 3페이지)
  - 의존성: Task 241 (프론트엔드 URL 교체 완료)
  - 규모: L
  - 에이전트: `주도` qa-test-expert | `협업` gas-backend-expert, seo-performance-expert
  - 구현 사항:
    ...
    - **Airtable 관리자 워크플로우**: 관리자가 Airtable GUI에서 직접 데이터 조회/수정할 수 있는지 확인 (뷰 동작, 필터, Linked Record 탐색)
    - **데이터 마이그레이션 검증** (GAS -> n8n 전환 시): Google Sheets 기존 데이터 CSV export -> Airtable import -> 무결성 확인
...
| C-3 | Airtable 기반 정산 데이터 무결성 | 설계 완료 (Linked Record + Rollup) | Task 201, Task 212 (v2.0) |
...
| Airtable 무료 한도 (1,200행) 초과 | 정산 내역 1,200건+ | Airtable Team 플랜($20/월) 또는 월별 아카이빙 |
| Airtable 유료 한도 (50,000행) 초과 | 대규모 확장 시 | n8n PostgreSQL 직접 테이블 또는 Supabase 전환 |
...
파트너 클래스 플랫폼 (선순환 엔진) -- n8n + Airtable 백엔드
