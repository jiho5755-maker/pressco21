...
- **Task 241: 프론트엔드 URL 교체 (3줄) + 메이크샵 3페이지 등록**
  ...
  - 구현 사항:
    - `파트너클래스/목록/js.js` L13: `GAS_URL` 값을 `https://n8n.pressco21.com/webhook/class-api`로 교체
    - `파트너클래스/상세/js.js` L14: `GAS_URL` 값을 `https://n8n.pressco21.com/webhook/class-api`로 교체
    - 메이크샵 공통 JS: `window.PRESSCO21_GAS_URL`을 `https://n8n.pressco21.com/webhook/partner-auth`로 교체
    - POST 요청 Content-Type 변경 검토: `text/plain` -> `application/json` (CORS 설정 완료 후)
    - 메이크샵 관리자에서 파트너클래스 3개 페이지 (목록/상세/파트너) 개별 페이지 등록
    - API_ENDPOINTS 매핑 객체 방식 적용 여부 검토 (action별 URL 분기 vs 단일 URL+action 파라미터)
  - 테스트 체크리스트:
...
- **Task 251: 전체 E2E 통합 테스트 + NocoDB 관리자 워크플로우 검증**
