...
- `입금 정합성 감사 경보`처럼 반복 가능한 모니터링성 알림은 은행 실시간 알림방으로 보내지 않습니다.
- 감사 경보는 `PRESSCO_AUDIT_CHAT_ID`를 우선 사용하고, 미설정 시 `TELEGRAM_CHAT_ID`로 보냅니다.
- `PRESSCO_BANK_CHAT_ID`는 실입금/거래/파싱 실패처럼 즉시 확인이 필요한 은행 운영 알림 전용으로 유지합니다.
