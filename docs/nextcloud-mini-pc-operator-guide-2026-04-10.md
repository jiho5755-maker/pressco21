...
- 관리자
  - `pressco21` (장지호)
  - `envyco` (이진선)
  - `00_INBOX`, `브랜드`, `디자인`, `사진`, `내부문서`, `영상자료`, `업무가이드`, `관리자문서함` 접근 가능
- 직원
  - `cardboardwindow` (장다경)
  - `tmdgo1993` (조승해)
  - `jhl9464` (이재혁)
  - `00_INBOX`, `브랜드`, `디자인`, `사진`, `내부문서`, `영상자료`, `업무가이드` 접근 가능

## 현재 폴더 구조

- `00_INBOX`
  - 어디에 넣어야 할지 애매한 자료를 받는 접수함이다.
  - `직원업로드`
  - `관리자검토`
  - `분류실패`
  - 실제 원본 경로: `/home/pressbackup/pressco21/nextcloud/shared/employee-uploads`
- `브랜드`
...
- `업무가이드`
  - AI Wiki와 공식 운영 기준을 직원이 읽기 쉬운 형태로 제공하는 읽기 전용 영역이다.
  - `자주묻는질문`
  - `표준답변`
  - `업무체크리스트`
  - `AI_Wiki_공식요약본`
  - 실제 원본 경로: `/home/pressbackup/pressco21/nextcloud/shared/library/work-guides`
- `관리자문서함`
  - 카테고리 루트이며 직접 업로드하지 않는다.
  - `매출-재무`
  - `세무-증빙`
...
- 위치를 모르겠는 자료는 `00_INBOX/직원업로드`에 넣는다.
- `영상자료`는 읽기/다운로드 용도다. 수동 업로드 라인으로 쓰지 않는다.
- `업무가이드`는 직원용 읽기 전용 결과물이다. AI Wiki 원본은 `관리자문서함/AI_Wiki_관리원본`에서 관리자만 관리한다.
- 계약, 세무, 회계, 인사, 고객 개인정보, 계정 정보는 모두 `관리자문서함` 아래로 보낸다.
- 파일명은 가능하면 `YYYYMMDD_분류_대상_내용_상태_v01` 형식으로 맞춘다.
- 상세 규칙은 `docs/nextcloud-file-classification-rules-2026-04-15.md`를 기준으로 한다.
...
- 위치가 애매한 파일은 `00_INBOX/직원업로드`에 업로드
...
- 직원용 폴더를 모두 확인 가능
- 추가로 `관리자문서함`에서 매출, 세무, 계약, 인사, 고객 개인정보, 계정 권한, AI Wiki 관리 원본을 보관
- `00_INBOX`를 검토해 공식 폴더, 업무가이드, 관리자문서함, 자동화 내부 경로로 분류
