...
| 레이어 | 기술 | 비고 |
|--------|------|------|
| 프론트엔드 | React + Vite + TypeScript | 컴포넌트 기반 모듈화 |
| UI 컴포넌트 | shadcn/ui + Tailwind CSS | 브랜드 컬러 `#7d9675` 커스터마이징 |
| 차트 | Recharts (shadcn/ui 기본) | React 친화적 차트 라이브러리 |
| 엑셀 | SheetJS (xlsx) | npm 패키지 |
| API 프록시 | n8n Webhook | NocoDB 토큰 서버사이드 격리 |
| 백엔드 | NocoDB (self-hosted) | nocodb.pressco21.com |
| 데이터베이스 | SQLite (NocoDB 내장) | Docker exec 직접 접근 가능 |
| 배포 | Oracle Cloud A1 Flex + Docker | 기존 인프라 재활용 |
...
### Phase 6: 회원 등급 체계 구축 + 고객 카드 강화

**[담당: ecommerce-business-expert]** (주관 — 등급 기준·수익성 설계)
**[협업: brand-planning-expert]** (배지 브랜딩, 커뮤니케이션 문구)
**[협업: sales-partnership-specialist]** (파트너스/VIP/엠버서더 전략, 협회 연계)
**[협업: partner-admin-developer]** (UI 구현)
**[협업: makeshop-ui-ux-expert]** (고객 카드 디자인)
**예상 소요**: 7~10일

> ⚠️ **Phase 6 착수 전 필수 사전 오케스트레이션**
>
> 아래 3개 에이전트를 병렬 실행하여 결과 취합 → 최종 규정 확정 후 구현 착수:
> - `ecommerce-business-expert`: 할인율 수익성 분석, 인센티브 공식, KPI 설계
> - `brand-planning-expert`: 등급 배지 디자인, 커뮤니케이션 문구 최종안
> - `sales-partnership-specialist`: 파트너스 자격 인정 기준, VIP 협상 전략, 엠버서더 계약안
>
> 참고 문서 (이미 작성된 가안):
> - `docs/member-grade-brand-strategy.md` (brand-planning-expert 산출물)
> - `.claude/agent-memory/sales-partnership-specialist/partner-vip-ambassador-strategy.md`

---

#### Phase 6-A: 등급 체계 설계 및 DB 스키마

**등급 구조 — 성장 경로 4단계 + 특별 칭호 1개**

| 등급 | 한글명 | 자격 조건 | 핵심 혜택 | 배지 색상 |
|------|--------|----------|----------|---------|
| 회원 | 일반회원 | 회원가입 | 정가 구매 | `#a8b5a0` 연한 세이지 |
| 인스트럭터 | 강사회원 | 일반 자격증/수료증 OR 원예 관련 사업자등록증 | 5~10% 할인, 1개 소매도 무조건 적용 | `#7d9675` 브랜드 그린 |
| 파트너스 | 제휴 회원 | 제휴 협회/업체 소속 OR 프레스코21 인정 자격증 | 10~20% 할인, 1개 소매도 무조건 적용 | `#5e8a6e` 딥 그린 |
| VIP | 최고 등급 | 프레스코21 직접 선정 (주로 협회장) | 특가 + 분기 인센티브 적립금 + 협회 행사 지원 + 전담 1:1 상담 | `#b89b5e` 골드 |

**특별 칭호 (성장 경로와 독립, 어떤 등급이든 병행 가능)**

| 칭호 | 자격 조건 | 핵심 혜택 | 배지 색상 |
|------|----------|----------|---------|
| 엠버서더 | 바이럴 능력 보유 판단 시 초청 | 신제품 협찬 + 전 제품 30% 할인 + 전용 할인코드 | `#8b6fae` 소프트 퍼플 |

> 엠버서더: 금전 보상 없음, 6개월 단위 협력 약정, 월 2건 포스팅, 전용코드로 ROI 측정
> 엠버서더 30% 할인 → 마진 CRITICAL 상품 적자 위험 ⚠️ → 협찬 대상 상품 사전 목록 제한 or 분기 협찬 수량 캡 필수

**NocoDB customers 테이블 신규 필드**

| 필드명 | 타입 | 설명 |
|--------|------|------|
| `member_grade` | SingleSelect | MEMBER / INSTRUCTOR / PARTNERS / VIP |
| `is_ambassador` | Checkbox | 엠버서더 칭호 병행 여부 |
| `ambassador_code` | SingleLineText | 엠버서더 전용 할인코드 |
| `grade_qualification` | LongText | 자격 근거 (자격증명, 협회명 등) |
| `discount_rate` | Number | 실 적용 할인율 (%) |
| `grade_approved_at` | DateTime | 등급 승인일 |
| `grade_updated_at` | DateTime | 최종 갱신일 |
| `annual_amount` | Number | 해당 연도 총 구매금액 (파생) |
| `annual_count` | Number | 해당 연도 총 거래 횟수 (파생) |

> 기존 강사회원 1,200명 → 마이그레이션 시 INSTRUCTOR로 일괄 분류 후 수동 검토
...
| 원칙 | 내용 |
|------|------|
| 순차 성장 | 회원 → 인스트럭터 → 파트너스 → VIP (단계 건너뜀 불가) |
| 협회 소속 파트너스 | 협회 MOU 체결 후 소속 회원 일괄 파트너스 등록 |
| 협회 탈퇴 시 | 파트너스 → 인스트럭터 자동 강등 (6개월 유예 기간) |
| 하향 보호 | 활동 감소 시 즉시 강등 없음 (6개월 유예) |
| 엠버서더 독립 | 어떤 등급이든 병행 가능, 성장 경로 무관 |

**할인율 (Phase 6 착수 전 ecommerce-business-expert 최종 확정 필요)**

- 인스트럭터: 5~10% (고정 단일 or 구매액 기준 차등, 메이크샵 회원 그룹 price_tier 연동)
- 파트너스: 10~20% (고정 단일 or 구매액 기준 차등, 메이크샵 회원 그룹 price_tier 연동)
- VIP: 개별 협상 특가 + 월 2회 전용 딜 (첫째/셋째 주 월요일)

**VIP 분기 인센티브 적립금 (메이크샵 process_reserve API 활용)**

- 협회원 분기 총 매출 집계 → 인센티브율 적용 → 협회장에게 적립금 지급
- 분기 실적 리포트 이메일 발송 (협회장 자발적 단톡방 공지 유도)
- 구체적 인센티브율: Phase 6 착수 전 sales-partnership-specialist + ecommerce-business-expert 협의
...
- `customer_type` 아이콘: 기관(🏛) / 협회(🤝) / 개인(👤) / 공방(🧵) / 강사(👩‍🏫)
- `customer_status` 배지: 초록(ACTIVE) / 노랑(DORMANT) / 빨강(CHURNED) / 회색(ARCHIVED)
- `member_grade` 배지: 위 색상 체계 적용 (회원/인스트럭터/파트너스/VIP)
- 엠버서더 칭호: ★ 별 아이콘 + `#8b6fae` 퍼플 보조 배지
- `outstanding_balance` > 0이면 빨간 원형 알림 표시
- RFM 점수 간이 표시 (Recency + Frequency + Monetary)

```javascript
// 등급 배지 색상 매핑
var GRADE_COLORS = {
  MEMBER:     { bg: '#a8b5a0', label: '회원' },
  INSTRUCTOR: { bg: '#7d9675', label: '인스트럭터' },
  PARTNERS:   { bg: '#5e8a6e', label: '파트너스' },
  VIP:        { bg: '#b89b5e', label: 'VIP' }
};
// 엠버서더 칭호: is_ambassador=true이면 ★ 아이콘 + #8b6fae 배지 추가 표시
```
...
- 기존 `address1` (단일) → `addresses` (JSON 배열)
- 형식: `[{"alias": "본원", "addr": "서울시..."}, {"alias": "분원1", "addr": "경기도..."}]`
- 명세표 작성 시 거래처 선택 후 배송지 별칭 드롭다운 선택 가능

---

**수락 기준 (Phase 6 전체)**

- [ ] Phase 6 착수 전 ecommerce-business-expert + brand-planning-expert + sales-partnership-specialist 오케스트레이션 결과 취합 완료
- [ ] member_grade 필드로 기존 강사회원 1,200명 INSTRUCTOR 자동 분류 및 수동 검토 완료
- [ ] 등급별 배지 색상 고객 카드에 정상 표시 (회원/인스트럭터/파트너스/VIP)
- [ ] 엠버서더 칭호 ★ 아이콘 병행 표시 (is_ambassador=true)
- [ ] 할인율 메이크샵 회원 그룹 연동 확인 (인스트럭터/파트너스)
- [ ] VIP 분기 인센티브 적립금 지급 시뮬레이션 성공
- [ ] 엠버서더 30% 할인 협찬 대상 상품 제한 목록 확정
- [ ] 배송지 별칭 저장 및 명세표 드롭다운 정상 작동
- [ ] RFM 점수 표시 (선택, 개발 여건에 따라 조정)
...
Phase 6 (1~1.5주) ── Phase 1-D 완료 후 병렬 ─────────────────
  [사전] 3개 에이전트 병렬 오케스트레이션 (착수 전 필수)
         ecommerce-business-expert: 할인율·인센티브·KPI 확정
         brand-planning-expert: 배지 디자인·커뮤니케이션 확정
         sales-partnership-specialist: 자격 기준·VIP·엠버서더 계약 확정
  6-A. 등급 DB 설계 [ecommerce-business-expert + devops-monitoring-expert]
  6-B. 등급 로직   [partner-admin-developer + ecommerce-business-expert]
  6-C. 고객 카드   [partner-admin-developer + makeshop-ui-ux-expert]
  6-D. 배송지 별칭  [partner-admin-developer]
