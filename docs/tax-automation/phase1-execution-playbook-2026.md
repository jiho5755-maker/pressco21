...
- [`templates/accountant_request_2025_mapping.csv`](./templates/accountant_request_2025_mapping.csv)

이 보드는 2025년 귀속 자료 수집을 위해 기본 담당자와 내부 마감일을 채운 CSV다.
세무사가 준 결산 요청자료 9개는 `accountant_request_2025_mapping.csv`에 원문 기준으로 매핑했고, 실행 보드에는 `TAX-2025-053`~`TAX-2025-061` 항목을 추가했다.
...
| 번호 | 세무사 요청자료 | 실행 보드 반영 |
|---|---|---|
| 1 | 통장, 카드사용내역 | 기존 `TAX-2025-020`, `TAX-2025-022`, `TAX-2025-023`으로 수집 |
| 2 | 대출금이자납입내역 | `TAX-2025-053` 추가 |
| 3 | 수입신고필증, 수입정산서 | `TAX-2025-054` 추가 |
| 4 | 전기요금납부내역서 | `TAX-2025-055` 추가 |
| 5 | 퇴직연금납부내역 | `TAX-2025-056` 추가 |
| 6 | 보험료납부내역, 화재·차량 보험증권 | `TAX-2025-057`, `TAX-2025-058`로 분리 추가 |
| 7 | 지방세세목별납부내역 | `TAX-2025-059` 추가 |
| 8 | 기부금영수증 | `TAX-2025-060` 추가 |
| 9 | 네이버광고충전내역 | `TAX-2025-061` 추가 |

주의:

- 위 항목은 세무사가 요청한 “자료 수집 단위”다.
- 비용 인정, 공제 가능 여부, 원금/이자 구분, 개인/사업 구분은 자동 판단하지 않고 세무사 확인 질문으로 남긴다.
