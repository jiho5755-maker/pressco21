...
강사회원/회원등급 가격 안내 팝업 이미지를 등록할 때는 파일명에 아래 키워드를 포함합니다.

```text
pc21_member_price_notice
```

예:

```text
pc21_member_price_notice_202604.gif
pc21_member_price_notice_202604.jpg
```

이 파일명 규칙을 지키면 스킨 JS가 해당 팝업을 회원가 안내 팝업으로 인식합니다.
현재 구현 기준으로 이 팝업은 강사회원 로그인 컨텍스트에서만 통합 팝업에 포함됩니다.
