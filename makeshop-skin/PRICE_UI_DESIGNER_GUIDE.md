...
따라서 이미지 파일명만으로 강사회원 전용 여부를 판단하면 누락될 수 있습니다.

현재 운영 규칙은 아래와 같습니다.

```text
이벤트 팝업 0번 슬롯(eventwindow0)은 강사회원/회원등급 가격 안내 전용으로 사용
```

주의: MakeShop이 업로드 이미지를 자동으로 `jewoo_event...` 이름으로 바꾸기 때문에, 파일명만으로 전용 팝업을 구분하지 않습니다. 현재 스킨은 `eventwindow0` 자체를 전용 안내 팝업으로 보고 비강사회원에게는 조기 제거합니다.
...
스킨 JS는 `eventwindow0` 또는 위 이미지명을 강사회원 안내 팝업으로 인식하고, 강사회원 로그인 컨텍스트에서만 통합 팝업에 포함합니다.

디자이너 작업 규칙:

1. 회원등급/가격 안내 팝업은 MakeShop 이벤트 팝업 0번 슬롯에 등록합니다.
2. 일반 고객 전체에게 보여야 하는 팝업은 0번 슬롯을 사용하지 않습니다.
3. 팝업 이미지를 교체한 뒤 일반회원에게 보이면 개발자에게 새 이미지명 확인을 요청합니다.
