...
(function() {
    ...
    var checkPartnerAndEducation = PC.auth.checkPartnerAuth;

    /** n8n WF-16 강의 등록 엔드포인트 */
    /* REGISTER_URL은 PC.api.fetchPost('CLASS_REGISTER', ...) 로 대체 */
    ...
    var CR_AREAS = ['crNoticeArea', 'crFormArea', 'crSuccessArea', 'crAlreadyArea'];
    ...
    function init() {
        ...
        // 회원 ID 읽기 (가상태그)
        memberId = PC.auth.getMemberId('crMemberId');

        // 미로그인 처리
        if (!memberId) {
            PC.ui.showArea('crNoticeArea', CR_AREAS);
            return;
        }
        ...
        checkPartnerAndEducation(memberId, function(status) {
            ...
            if (status === 'SUSPENDED') {
                showAlreadyArea(
                    '파트너 계정이 비활성 상태입니다',
                    '현재 파트너 계정이 정지 또는 비활성 상태예요. 자세한 내용은 고객센터로 문의해주세요.'
                );
                return;
            }
            if (status === 'PENDING') {
                showAlreadyArea(
                    '파트너 승인 대기 중입니다',
                    '파트너 신청이 심사 중이에요. 승인 후 강의 등록이 가능합니다. 1~3 영업일 내 이메일로 결과를 안내드립니다.'
                );
                return;
            }
            if (status === 'ERROR') {
                showAlreadyArea(
                    '일시적인 오류가 발생했습니다',
                    '네트워크 연결을 확인하신 후 페이지를 새로고침 해주세요. 문제가 계속되면 고객센터로 문의해주세요.'
                );
                return;
            }
            // 정상: 폼 표시
            PC.ui.showArea('crFormArea', CR_AREAS);
            bindFormEvents();
        });
    }
    ...
    function bindFormEvents() {
        ...
        // 구조화 템플릿 토글
        var templateCheckbox = document.getElementById('crUseTemplate');
        var templateFields = document.getElementById('crTemplateFields');
        if (templateCheckbox && templateFields) {
            templateCheckbox.addEventListener('change', function() {
                templateFields.style.display = this.checked ? '' : 'none';
            });
        }
    ...
    /**
     * 구조화 템플릿 섹션 수집
     * @returns {Array|null} null이면 템플릿 미사용
     */
    function collectTemplateSections() {
        var checkbox = document.getElementById('crUseTemplate');
        if (!checkbox || !checkbox.checked) return null;

        var sections = [];
        var fields = [
            { id: 'crWhatToLearn', key: 'what_to_learn', label: '이 수업에서 배울 것' },
            { id: 'crHowItWorks', key: 'how_it_works', label: '수업 진행 방식' },
            { id: 'crRecommendedFor', key: 'recommended_for', label: '추천 대상' },
            { id: 'crExtraInfo', key: 'extra_info', label: '추가 안내' }
        ];
    ...
    function collectFormData() {
        ...
        return {
            ...
            template_sections:  collectTemplateSections()
        };
    }
    ...
    function postRegister(data, callback) {
        PC.api.fetchPost('CLASS_REGISTER', data)
            .then(function(resData) {
                callback(null, resData);
            })
            .catch(function(err) {
                console.error('[ClassRegister] 강의 등록 API 실패:', err.code || '', err.message || err);
                callback(err, null);
            });
    }
    ...
    function showSuccessArea(classId) {
        ...
        PC.ui.showArea('crSuccessArea', CR_AREAS);
    ...
    function showAlreadyArea(title, desc) {
        ...
        PC.ui.showArea('crAlreadyArea', CR_AREAS);
        window.scrollTo({ top: 0, behavior: 'smooth' });
    }
    ...
    function showLoading() {
        PC.ui.showLoading('crLoadingOverlay');
    }

    function hideLoading() {
        PC.ui.hideLoading('crLoadingOverlay');
    }
