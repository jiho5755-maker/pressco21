...
(function() {
    ...
    function pc21GetBranduid(card) {
        var link = card ? card.querySelector('a.link[href*="branduid="], a[href*="branduid="]') : null;
        var match;
        if (!link) {
            return '';
        }
    ...
    function pc21FindMainPriceWrap(card) {
        var wraps = card ? card.querySelectorAll('.prds--price-wrap') : [];
        var i;
        for (i = 0; i < wraps.length; i += 1) {
            if (wraps[i].querySelector('.price') && !wraps[i].querySelector('.coupon')) {
                return wraps[i];
            }
        }
        return null;
    ...
    function pc21EnsureStrong(parent) {
        var strong = parent ? parent.querySelector('strong') : null;
        if (!strong && parent) {
            strong = document.createElement('strong');
            parent.insertBefore(strong, parent.firstChild);
        }
        return strong;
    }

    function pc21EnsureOriginal(prices, afterNode) {
        var original = prices ? prices.querySelector('.original') : null;
        var valueNode;
        if (!original && prices) {
            original = document.createElement('span');
            original.className = 'original';
            valueNode = document.createElement('span');
            original.appendChild(valueNode);
            original.appendChild(document.createTextNode('원'));
            if (afterNode && afterNode.parentNode) {
                afterNode.parentNode.insertBefore(original, afterNode.nextSibling);
            } else {
    ...
    function pc21EnsureDiscount(prices, afterNode) {
        var node = prices ? prices.querySelector('.pc21-ug-discount, .discount') : null;
        if (!node && prices) {
            node = document.createElement('span');
            node.className = 'pc21-ug-discount';
            if (afterNode && afterNode.parentNode) {
    ...
    function pc21HasVisibleDiscount(prices) {
        var saleNode = prices ? prices.querySelector('.price') : null;
        var baseNode = prices ? prices.querySelector('.original') : null;
        var sale = pc21ParseWon(saleNode ? saleNode.textContent : '');
        var base = pc21ParseWon(baseNode ? baseNode.textContent : '');
    ...
    function pc21ApplyMemberPolicyToCard(card) {
        ...
        var wrap = pc21FindMainPriceWrap(card);
        var prices = wrap ? wrap.querySelector('.prices') : null;
        var priceNode = prices ? prices.querySelector('.price') : null;
        var currentPrice = pc21ParseWon(priceNode ? priceNode.textContent : '');
        var priceStrong;
        var originalNode;
        var originalValueNode;
        var discountNode;
        var applyKey;

        if (!policy || !prices || !priceNode || !policy.member || !policy.base || policy.member >= policy.base) {
            return;
        }
        if (pc21HasVisibleDiscount(prices)) {
            return;
        }
        ...
        if (wrap.getAttribute('data-pc21-member-applied') === applyKey) {
            return;
        }

        priceStrong = pc21EnsureStrong(priceNode);
        originalNode = pc21EnsureOriginal(prices, priceNode);
        originalValueNode = originalNode ? originalNode.querySelector('span') : null;
        discountNode = pc21EnsureDiscount(prices, originalNode);

        if (priceStrong) {
            priceStrong.textContent = pc21FormatNumber(policy.member);
        }
        if (originalValueNode) {
            originalValueNode.textContent = pc21FormatNumber(policy.base);
        }
        ...
        wrap.setAttribute('data-pc21-member-applied', applyKey);
        wrap.setAttribute('data-pc21-member-source', 'member-duplicate-policy-20260420');
    }
    ...
    function pc21ApplyMemberPolicies(root) {
        ...
        cards = scope.querySelectorAll('.prds-list--item');
        for (i = 0; i < cards.length; i += 1) {
            pc21ApplyMemberPolicyToCard(cards[i]);
        }
        if (scope.matches && scope.matches('.prds-list--item')) {
