...
/** 다음 상태 전환 매핑 */
const NEXT_STATUS: Partial<Record<TaskStatus, { label: string; next: TaskStatus }>> = {
  todo: { label: "시작", next: "in_progress" },
  in_progress: { label: "검토", next: "review" },
  review: { label: "완료", next: "done" },
};

interface TaskCardProps {
  task: Task;
  onQuickAction?: (taskId: number, newStatus: TaskStatus) => void;
  compact?: boolean;
}
...
export function TaskCard({ task, onQuickAction, compact = false }: TaskCardProps) {
  ...
  return (
    <Card
      className="p-3 cursor-pointer active:scale-[0.98] transition-transform duration-150"
        ...
        {transition && onQuickAction && (
          <Button
            ...
            onClick={(e) => {
              ...
              onQuickAction(task.id, transition.next);
            }}
          >
            {transition.label}
          </Button>
