...
import {
  ...
  X, CalendarClock, Briefcase, User, CalendarDays, Calendar
} from "lucide-react";
...
export function CalendarPage() {
  ...
  useEffect(() => { setPersonalEvents(getPersonalEvents()); }, []);
  ...
  const taskCountByDate = useMemo(() => {
    ...
    if (mode === "company") {
      for (const [key, dayTasks] of companyTasksByDate) {
        map.set(key, { total: dayTasks.length, urgent: dayTasks.some((t) => t.priority === "p1"), high: dayTasks.some((t) => t.priority === "p2") });
      }
    } else {
      for (const [key, dayEvents] of personalByDate) {
        map.set(key, { total: dayEvents.length, urgent: false, high: false });
      }
    }
    return map;
  ...
  // 주간 뷰일 때
  if (viewType === "week" && mode === "company") {
    return (
      <div className="flex flex-col min-h-screen">
        <Header title="캘린더" showBack rightAction={
          <div className="flex gap-0.5">
            <Button size="icon" variant="ghost" className="h-8 w-8" onClick={() => setViewType("month")}>
              <CalendarDays className="h-4 w-4" />
            </Button>
          </div>
        } />
        <main className="max-w-[480px] mx-auto w-full px-4 py-3 flex-1">
          <ToggleGroup value={[mode]} onValueChange={(v) => { const n = v[v.length-1] as CalendarMode; if (n) setMode(n); }}
            className="h-9 bg-muted/50 rounded-xl p-0.5 mb-3 w-full">
            <ToggleGroupItem value="company" className="flex-1 text-xs h-8 rounded-lg gap-1 data-[state=on]:bg-card data-[state=on]:shadow-sm"><Briefcase className="h-3.5 w-3.5" />회사 업무</ToggleGroupItem>
            <ToggleGroupItem value="personal" className="flex-1 text-xs h-8 rounded-lg gap-1 data-[state=on]:bg-card data-[state=on]:shadow-sm"><User className="h-3.5 w-3.5" />내 일정</ToggleGroupItem>
          </ToggleGroup>
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <Header title="캘린더" showBack rightAction={
        <div className="flex gap-0.5">
          {mode === "company" && (
            <Button size="icon" variant="ghost" className="h-8 w-8" onClick={() => setViewType(viewType === "month" ? "week" : "month")}>
              {viewType === "month" ? <Calendar className="h-4 w-4" /> : <CalendarDays className="h-4 w-4" />}
            </Button>
          )}
        </div>
      } />
          ...
          <>
            ...
            <div className="mb-4">
              <div className="flex items-center justify-between mb-2.5">
                ...
                {mode === "company" ? (
                  <Button size="sm" variant="outline" className="h-7 text-[11px] px-2.5 rounded-lg gap-1" onClick={() => navigate("/tasks/new")}>
                    <Plus className="h-3 w-3" />업무 추가
                  </Button>
                ) : (
                  <Button size="sm" variant="outline" className="h-7 text-[11px] px-2.5 rounded-lg gap-1" onClick={() => setShowAddForm(true)}>
              ...
              {showAddForm && mode === "personal" && (
                <div className="mb-3 p-3 bg-card rounded-xl border border-primary/30 space-y-2.5">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-[11px] text-muted-foreground">시간대:</span>
                    <div className="flex gap-1">
                      {(["allday", "morning", "afternoon"] as TimeSlot[]).map((slot) => (
                        <Badge key={slot} variant={newTimeSlot === slot ? "default" : "outline"}
                          className="cursor-pointer text-[10px] rounded-full px-2 py-0.5"
                    ...
                    <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => { setShowAddForm(false); setNewEventTitle(""); }}><X className="h-3.5 w-3.5" /></Button>
                  </div>
                </div>
              )}
              ...
              {mode === "company" && (
                selectedCompanyTasks.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-8 text-muted-foreground/50">
                    <Inbox className="h-7 w-7 mb-1.5 opacity-40" /><p className="text-xs">이 날 예정된 업무가 없습니다</p>
                  </div>
                ) : (
                  <div className="space-y-2">
                    {selectedCompanyTasks
                      .sort((a, b) => { if (a.priority === "p1" && b.priority !== "p1") return -1; if (a.priority !== "p1" && b.priority === "p1") return 1; return 0; })
              ...
              {mode === "personal" && (
                selectedPersonalEvents.length === 0 && !showAddForm ? (
                  <div className="flex flex-col items-center justify-center py-8 text-muted-foreground/50">
                    <Inbox className="h-7 w-7 mb-1.5 opacity-40" /><p className="text-xs">이 날 등록된 일정이 없습니다</p>
                    <button type="button" className="text-xs text-primary mt-2 font-medium" onClick={() => setShowAddForm(true)}>+ 일정 추가하기</button>
                  </div>
                ) : (
                  <div className="space-y-1.5">
                    {selectedPersonalEvents
                      .sort((a, b) => { const order: Record<string, number> = { morning: 0, afternoon: 1, allday: 2 }; return (order[a.timeSlot] ?? 2) - (order[b.timeSlot] ?? 2); })
                      .map((event) => (
                        <div key={event.id} className="flex items-center gap-2.5 p-2.5 bg-card rounded-xl border border-border/60">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <span className="text-[10px] text-blue-500">{TIME_SLOT_LABELS[event.timeSlot ?? "allday"]}</span>
                          </div>
                          <button type="button" className="text-muted-foreground/40 hover:text-destructive transition-colors p-1" onClick={() => handleDeletePersonalEvent(event.id)}>
