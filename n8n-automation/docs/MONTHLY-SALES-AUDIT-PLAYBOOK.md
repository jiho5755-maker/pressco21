...
- 일자별 조회 가능
- 확정 기준: `order_status` 또는 `ord_state`가 `Y` 또는 `S`
  - `Y`: 결제완료
  - `S`: 배송/처리 진행 후에도 결제 완료 매출로 유지
  - `N`/`C` 등 미입금·취소 상태는 제외
...
- F23 메이크샵 일일 집계에서 `Y/S` 상태만 매출로 인정하고 개인결제 카테고리 097은 CRM 강매칭 시 제외
- F26 메이크샵 백필에서도 같은 `Y/S` 기준과 개인결제 중복 제거를 적용
- 매칭된 개인결제건은 MakeShop 주문일이 아니라 CRM 거래명세표 `invoice_date` 기준으로 1회 귀속한다
- 기관/거래처 요청으로 명세표 발송일을 다음 영업일 등으로 잡은 경우에도 CRM `invoice_date`가 매출 귀속일이다
...
F23 일일 자동 보고의 쿠팡 금액은 `createdAt` 기준 운영 매출이다. 당일에는 쿠팡 취소/반품/정산 변동이 늦게 반영될 수 있으므로 `정산 매출`로 확정하지 않는다.

### 일일 보고 기준

- 주문 생성일 기준으로 쿠팡 주문 품목을 집계한다.
- `canceled=true`, `cancelCount`, `holdCountForCancel` 등 주문 품목에 이미 표시된 취소/보류 수량은 제외한다.
- API 오류, 14:30~19:00 호출금지, 429/403 발생 시 0원으로 덮지 않는다.
- 기존 확정값이 있으면 유지하고, 없으면 `coupang_collection_status=pending_backfill`로 남긴다.

### 월마감 기준

- D+3~D+7에 쿠팡 취소/반품/정산 원장과 일일 집계값을 대조한다.
- 취소/반품/환불이 뒤늦게 확정된 주문은 월마감 조정표에서 차감한다.
- 월마감 확정 전에는 리포트 표현을 `주문 기준 매출`로 유지한다.
- 월마감 확정 후에만 `정산 반영 매출`로 표기한다.

### 체크리스트 추가

- [ ] 쿠팡 `coupang_collection_status`가 `confirmed`인가?
- [ ] `pending_backfill`/`api_error_*` 날짜가 남아 있지 않은가?
- [ ] 쿠팡 취소/반품/환불 원장과 D+3~D+7 재대조를 완료했는가?
- [ ] 일일 주문 기준 매출과 월마감 정산 반영 매출을 문구상 구분했는가?
