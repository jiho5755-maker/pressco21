  ...
  const invoices = [
    ...
    },
    {
      ...
      memo: `${INVOICE_META_PREFIX} ${JSON.stringify({
        ...
        revenuePostingStatus: 'pending',
        internalMemo: '서류작성용, 실제거래건 x',
        paymentHistory: [],
      })}`,
    },
    {
      Id: 8004,
      invoice_no: 'GOV-SMOKE-NONTRADE',
      invoice_date: today,
      customer_id: 7002,
      customer_name: '제시카플로라',
      receipt_type: DEFAULT_RECEIPT_TYPE,
      ...
      payment_status: 'unpaid',
      status: 'unpaid',
      memo: `서류작성용, 실제거래건 x\n${INVOICE_META_PREFIX} ${JSON.stringify({
        depositUsedAmount: 0,
        discountAmount: 0,
        internalMemo: '서류작성용, 실제거래건 x',
        paymentHistory: [],
...
test('T14-01: 핵심 거버넌스 화면을 브라우저에서 순회 검증한다', async ({ page }) => {
  ...
  await expect(page.getByText('GOV-SMOKE-DRAFT')).toHaveCount(0)
  await expect(page.getByText('GOV-SMOKE-NONTRADE')).toHaveCount(0)
  ...
  const reviewRow = page.locator('tbody tr', { hasText: '김순자' }).first()
  await expect(reviewRow).toBeVisible()
  await reviewRow.getByRole('button', { name: '고객 지정' }).click()
  await expect(page.getByRole('dialog', { name: '입금행 고객 매핑' })).toBeVisible()
  await expect(page.getByLabel('입금자명')).toHaveValue('김순자')
  await expect(page.getByLabel('입금액')).toHaveValue('220000')
  await page.getByRole('button', { name: '닫기' }).click()

  await page.getByRole('button', { name: '수동 입금 반영' }).click()
  await expect(page.getByRole('dialog', { name: '수동 입금 반영' })).toBeVisible()
  await page.getByLabel('입금자명').fill('김순자')
  await page.getByLabel('입금액').fill('1000000')
  await page.getByLabel('대상 고객 검색').fill('제시카')
  await expect(page.getByRole('button', { name: /제시카플로라/ })).toBeVisible()
  await page.getByRole('button', { name: '닫기' }).click()
