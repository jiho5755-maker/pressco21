...
test('T14-01: 핵심 거버넌스 화면을 브라우저에서 순회 검증한다', async ({ page }) => {
  ...
  await page.goto('/receivables?asOf=2026-04-30')
  await expect(page.getByRole('heading', { name: /수금 관리/ })).toBeVisible()
  ...
  await expect(page.getByRole('heading', { name: '월말점검' })).toBeVisible()
