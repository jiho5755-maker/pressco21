...
function buildTelegramSummaryCode() {
  return [
    ...
    "function formatAmount(value) { return (Number(value) || 0).toLocaleString() + '원'; }",
    "function formatDate(value) { return String(value || '').replace('T', ' ').slice(0, 16) || '-'; }",
    "function recommendAlias(value) { return String(value || '').trim(); }",
    ...
    "  '계좌: ' + accountLabel,",
    '];',
    "deposits.slice(0, 5).forEach((entry) => {",
    "  const sender = String(entry.sender || '미상').trim() || '미상';",
    "  const balanceSuffix = Number(entry.balance) > 0 ? ' / 잔액 ' + formatAmount(entry.balance) : '';",
    "  lines.push('- 입금 ' + sender + ' / ' + formatAmount(entry.amount) + ' / ' + formatDate(entry.occurredAt) + balanceSuffix);",
    "});",
    '',
    'if (exactActions.length > 0) {',
    "  lines.push('', '처리: 자동반영 완료');",
    '  exactActions.slice(0, 5).forEach((entry) => {',
    "    const target = entry.kind === 'invoice'",
    "      ? (entry.invoiceNo ? '명세표 ' + entry.invoiceNo : 'CRM 명세표')",
    "      : '기존 장부';",
    "    lines.push('- ' + (entry.customerName || entry.sender || '미상') + ' / ' + target + ' / 잔액 ' + formatAmount(entry.remainingOutstanding));",
    '  });',
    '}',
    '',
    'if (reviewEntries.length > 0) {',
    "  lines.push('', '처리: 검토 필요');",
    '  reviewEntries.slice(0, 3).forEach((entry) => {',
    "    const candidates = Array.isArray(entry.candidates) ? entry.candidates.slice(0, 2).map((candidate) => candidate.customerName || candidate.invoiceNo || candidate.kind).filter(Boolean).join(', ') : '';",
    "    const alias = recommendAlias(entry.sender);",
    "    const aliasText = alias ? ' / 추천별칭 ' + alias : '';",
    "    lines.push('- ' + (entry.sender || '미상') + ' / ' + formatAmount(entry.amount) + (candidates ? ' / 후보 ' + candidates : '') + aliasText);",
    '  });',
    '}',
    '',
    'if (unmatchedEntries.length > 0) {',
    "  lines.push('', '처리: 미매칭');",
    '  unmatchedEntries.slice(0, 3).forEach((entry) => {',
    "    const alias = recommendAlias(entry.sender);",
    "    const aliasText = alias ? ' / 추천별칭 ' + alias : '';",
    "    lines.push('- ' + (entry.sender || '미상') + ' / ' + formatAmount(entry.amount) + (entry.reason ? ' / ' + entry.reason : '') + aliasText);",
    '  });',
    '}',
    '',
    'if (duplicateEntries.length > 0 && (exactActions.length > 0 || reviewEntries.length > 0 || unmatchedEntries.length > 0)) {',
    "  lines.push('', '처리: 중복 무시');",
    "  lines.push('- 동일 입금 이벤트 ' + String(duplicateEntries.length) + '건은 재처리하지 않았습니다.');",
    '}',
    '',
    'if (deposits.length > 5) {',
    "  lines.push('', '... 외 ' + String(deposits.length - Math.min(deposits.length, 5)) + '건');",
    '}',
    "sentMap[summaryKey] = new Date().toISOString();",
    "const sentKeys = Object.keys(sentMap);",
    "if (sentKeys.length > 300) {",
