...
function buildCrmDepositAuditReportCode() {
  return [
    ...
    "  const parts = [",
    "    `NH메일 ${bucket.nhMail}`,",
    "    `파싱실패 ${bucket.parseFailures}`,",
    "    `입금 ${bucket.deposits}`,",
    "    `출금 ${bucket.withdrawals}`,",
    "  ];",
    "  if (bucket.parseFailuresUnsent > 0) parts.push(`미전송 파싱실패 ${bucket.parseFailuresUnsent}`);",
    "  if (bucket.failed > 0) parts.push(`failed ${bucket.failed}`);",
    "  if (bucket.pending > 0) parts.push(`pending ${bucket.pending}`);",
    "  return parts.join(' / ');",
    "}",
    "",
    ...
    "    pushIssue('daily-mismatch::' + day, 'high', `일일 정합성 주의 ${day}`, summarizeBucket(bucket));",
    "  }",
    "}",
    "",
    ...
    "  `요약: NH메일 ${recentMail.length}건 / 거래 ${recentTx.length}건 / failure ${recentFailureLog.length}건`,",
    "  '',",
    "];",
