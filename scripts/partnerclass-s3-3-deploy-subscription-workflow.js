...
var FILE_PATH = '파트너클래스/n8n-workflows/WF-SUB-subscription-kit-pilot.json';
...
async function main() {
    ...
    backupDir = path.join(OUTPUT_ROOT, getTimestamp() + '-s3-3-subscription');
    fs.mkdirSync(backupDir, { recursive: true });

    if (existing && existing.id) {
        writeJson(path.join(backupDir, existing.id + '-WF-SUB-subscription-kit-pilot.json'), await getWorkflow(apiKey, existing.id));
