...
var FILE_PATH = '파트너클래스/n8n-workflows/WF-CONTENT-affiliation-content-import.json';
...
async function main() {
    ...
    var absolutePath = path.join(REPO_ROOT, FILE_PATH);
    var localWorkflow = readJson(absolutePath);
    var workflows = null;
    var workflowIndex = {};
    var existing = null;
    var remoteWorkflow = null;
    var backupDir = '';
    ...
    existing = workflowIndex[localWorkflow.name] || (localWorkflow.id ? { id: localWorkflow.id } : null);
    backupDir = path.join(OUTPUT_ROOT, getTimestamp() + '-s3-5-content-import');
    fs.mkdirSync(backupDir, { recursive: true });

    if (existing && existing.id) {
        writeJson(path.join(backupDir, existing.id + '-WF-CONTENT-affiliation-content-import.json'), await getWorkflow(apiKey, existing.id));
