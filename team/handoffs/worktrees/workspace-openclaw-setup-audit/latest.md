...
## Codex 후속 조사 업데이트 (2026-04-24 13:30 KST)

**상태**: 남은 리스크 2개를 read-only로 원인 좁힘. 런타임 중지, 토큰 재발급, 설정 수정은 아직 수행하지 않음.

### Telegram `getUpdates` 409 conflict

**결론**: 같은 Telegram bot token을 두 long-poller가 동시에 사용 중이다.

- Mac 로컬 poller: LaunchAgent `com.pressco21.flora-telegram-room-router`.
  - 실행 파일: `/Users/jangjiho/workspace/pressco21/openclaw-project-hub/06_scripts/run-flora-telegram-room-router.js`.
  - config: `openclaw-project-hub/04_reference_json/flora-telegram-room-router.config.json`.
  - 해당 script는 `getUpdates`를 직접 호출한다.
- Flora 서버 poller: user service `openclaw-gateway.service`.
  - `journalctl --user -u openclaw-gateway.service`에서 `getUpdates conflict` 반복 확인.
- 로컬 `com.pressco21.flora-local-dev-worker`는 Telegram API/token을 쓰지만 `getUpdates` poller는 아니다.
- 로컬 `FLORA_TELEGRAM_BOT_TOKEN`, `PRESSCO21_DEV_ROUTER_BOT_TOKEN`, Flora gateway token의 fingerprint가 모두 `55d2ff8b123e`로 일치했다. 토큰값은 기록하지 않음.

**권장 조치**:

1. 운영 단일 poller는 Flora `openclaw-gateway.service`로 두고, legacy Mac `com.pressco21.flora-telegram-room-router`를 stop/unload한다.
2. room-router가 계속 필요하면 primary bot token을 공유하지 말고 별도 dev bot token으로 분리한다.
3. 조치 후 로컬 `flora-telegram-room-router` 로그와 Flora gateway journal에서 409가 사라졌는지 2~3 polling cycle 이상 확인한다.

### OpenAI Codex OAuth refresh failure

**결론**: Flora gateway auth store가 stale OpenAI Codex profile을 선택 중이다. 단순 네트워크 문제보다 토큰 재사용/프로필 오염 쪽이 유력하다.

- Flora gateway 로그에 `code="refresh_token_reused"`와 `Deprecated profile. Run openclaw models auth login --provider openai-codex`가 반복됨.
- `main`, `staff`, `flora-codex-room`, `flora-claude-room`, `flora-crm` 등 여러 agent의 `openai-codex:codex-cli`가 같은 stale refresh fingerprint(`277a868bbc11`)와 만료된 access expiry(2026-04-10 UTC)를 공유한다.
- `owner` agent에는 더 새 profile `openai-codex:jiho5755@gmail.com`이 존재하지만, `auth-state.json`의 `lastGood.openai-codex`는 여전히 stale `openai-codex:codex-cli`를 가리킨다.
- `openclaw models auth order get --provider openai-codex`는 주요 agents에서 비어 있어 명시 override도 없음.

**권장 조치**:

1. Flora `~/.openclaw/agents/*/agent/auth-profiles.json`, `auth-state.json`, `~/.openclaw/openclaw.json`을 백업한다.
2. TTY 가능한 세션에서 `openclaw models auth login --provider openai-codex`로 재인증한다.
3. fresh profile을 각 주요 agent의 OpenAI Codex auth order/lastGood에 반영한다. 예: `openclaw models auth order set --agent flora-frontdoor --provider openai-codex <fresh-profile-id>`.
4. `openai-codex/gpt-5.4` 요청이 Gemini fallback 없이 성공하는지 확인한다.

### 다음 실행 우선순위

1. 사용자 승인 후 Mac legacy room-router stop/unload 또는 dev bot token 분리.
2. Flora OpenAI Codex 재인증 및 stale profile 정리.
3. 성공 후 `OPS_LOG.md`, CTO playbook, 이 handoff에 검증 결과 추가.

## Codex 실행 업데이트 (2026-04-25 00:53 KST)

**상태**: 남은 리스크 2개 런타임 조치 및 검증 완료.

### Telegram `getUpdates` 409 conflict — 해결

- Mac legacy LaunchAgent `com.pressco21.flora-telegram-room-router`를 `bootout` 후 `disable` 처리.
- 운영 primary bot long-poller는 Flora `openclaw-gateway.service` 단일 경로로 유지.
- 검증:
  - `launchctl print-disabled gui/$(id -u)`에서 해당 label disabled 확인.
  - 로컬 router error log mtime이 disable 이후 증가하지 않음.
  - Flora gateway journal 최신 5분 창에서 Telegram `409`/`Conflict` 0건.
- Rollback:
  - `launchctl enable gui/$(id -u)/com.pressco21.flora-telegram-room-router`
  - `launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.pressco21.flora-telegram-room-router.plist`

### OpenAI Codex OAuth stale profile — 해결
...
- 브라우저 OAuth code 교환은 one-time code 소비/교환 실패로 중단.
- 대체 경로로 로컬 Codex CLI OAuth login을 서버 임시 `CODEX_HOME`에 복사해 `import-codex-cli`로 OpenClaw canonical profile `openai-codex:default` 생성. 임시 `CODEX_HOME`은 삭제 완료. 토큰값은 출력/기록하지 않음.
- `owner`, `main`, `staff`, `flora-codex-room`, `flora-claude-room`, `flora-crm`, `flora-frontdoor`에 `openai-codex:default`를 배포하고 `order.openai-codex`/`lastGood.openai-codex`를 해당 profile로 고정.
- stale/deprecated `openai-codex:codex-cli` 제거. `main` store의 legacy `google:gemini-api` type `api-key`는 현재 스키마 `api_key`로 정규화.
- 검증:
  - 대상 agent들의 `openclaw models --status-plain` → `openai-codex/gpt-5.4`.
  - `openclaw infer model run --local --model openai-codex/gpt-5.4` → `OK` for `owner`, `flora-codex-room`, `flora-frontdoor`.
  - `openclaw infer model run --gateway --model openai-codex/gpt-5.4` → `OK` for `flora-frontdoor`.
  - Flora gateway journal 최신 5분 창에서 `refresh_token_reused`, OAuth refresh/token-exchange error, invalid auth-profile warning 모두 0건.

### 남은 리스크

- `com.pressco21.flora-telegram-room-router`가 필요한 개발 워크플로우가 있으면 primary bot token 공유가 아니라 별도 dev bot token으로 분리해야 함.
- 로컬 Codex CLI OAuth token 자체 만료일은 2026-05-01 UTC로 확인됨. 이후에는 동일 절차로 fresh login/import 또는 OpenClaw 브라우저 OAuth 재시도가 필요할 수 있음.

## Codex 마감 업데이트 (2026-04-25 03:53 KST)

**상태**: OpenClaw/clawdbot setup audit 작업 마감 확인.

- `work/workspace/openclaw-setup-audit` 브랜치의 런타임 조치 커밋 `d631c5b`는 main merge `a3e44dc`를 통해 main에 포함됨.
- feature branch도 `origin/work/workspace/openclaw-setup-audit`에 fast-forward push 완료.
- 현재 워크트리 상태는 clean이며, `.omx/state/team` 기준 활성 team/mailbox 파일은 없음.
- 재검증:
  - Mac `com.pressco21.flora-telegram-room-router` LaunchAgent는 disabled 상태.
  - 로컬 `run-flora-telegram-room-router` 프로세스 없음.
  - Flora gateway journal 최근 10분 창에서 Telegram `409`/`Conflict`, `refresh_token_reused`, token-exchange, invalid auth-profile 경고 없음.
  - Flora `openclaw models --status-plain --agent flora-frontdoor` → `openai-codex/gpt-5.4`.
  - Flora `openclaw infer model run --gateway --model openai-codex/gpt-5.4 --prompt "Reply exactly: OK"` → `OK`.

### 최종 남은 운영 메모

- 로컬 Codex CLI OAuth token 만료일이 2026-05-01 UTC이므로, 이후 장애가 재발하면 fresh login/import 또는 OpenClaw 브라우저 OAuth 재시도 필요.
- legacy room-router를 다시 써야 한다면 primary bot token 공유 금지. 별도 dev bot token으로 분리 후 enable/bootstrap할 것.
