...
document.addEventListener("DOMContentLoaded", function() {
    const easyPurchaseBtn = document.querySelector('.btn-easy-purchase');
    ...
    if (easyPurchaseBtn) {
        easyPurchaseBtn.addEventListener('click', function(e) {
            ...
            // 카카오페이 버튼 찾기
            const kakaopayContainer = document.querySelector('#kakaopay_order_btn, .easyPayBtnWrap [id*="kakaopay"]');

            if (kakaopayContainer) {
                // 카카오페이 버튼의 실제 링크나 버튼 찾기
                const kakaopayButton = kakaopayContainer.querySelector('a, button, [onclick]');
