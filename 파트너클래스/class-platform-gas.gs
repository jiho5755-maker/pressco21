 ...
 * Task 201: Phase 2-A 데이터 구조 + GAS 기초 API
 * =============================================
 *
 * 역할:
 ...
 * - getCategories: 카테고리 목록 (공개, 캐시 24시간)
 * - health: 헬스 체크
 *
 ...
 * - pollOrders: 주문 폴링 (시간 트리거 또는 수동 호출)
 * - updateClassStatus: 클래스 상태 변경 (파트너 인증 필수)
 * - clearCache: 캐시 전체 삭제 (관리자용)
 *
 * @param {Object} e - HTTP 요청 객체
 * @return {TextOutput} JSON 응답
 */
...
function sendReminderIfNeeded_(row, type, rowNum, headers) {
  ...
  // 수강생 이메일 (정산 내역에 직접 저장하지 않으므로 주문 정보에서 추출 필요)
  // 현재 구조에서는 member_id로 회원 조회가 필요하지만,
  // Sheets에 student_email 컬럼이 없으므로 로그에만 기록
  // TODO: 향후 정산 내역에 student_email 컬럼 추가 시 활성화

  var daysLabel = type === 'D3' ? '3일' : '1일';
  var subject = '[PRESSCO21] 수업 ' + daysLabel + ' 전 안내 - ' + classData.class_name;

  var body = '<div style="font-family: \'Noto Sans KR\', sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">'
    + '<h2 style="color: #2d2d2d; border-bottom: 2px solid #b89b5e; padding-bottom: 12px;">수업 안내</h2>'
    + '<p style="color: #555;">수업이 ' + daysLabel + ' 남았습니다!</p>'
    + '<table style="width: 100%; border-collapse: collapse; margin: 16px 0;">'
    + '<tr style="background: #f8f5f0;"><td style="padding: 10px; font-weight: bold;">클래스</td>'
    + '<td style="padding: 10px;">' + classData.class_name + '</td></tr>'
    + '<tr><td style="padding: 10px; font-weight: bold;">일시</td>'
    + '<td style="padding: 10px;">' + (row.class_date || '') + '</td></tr>'
    + '<tr style="background: #f8f5f0;"><td style="padding: 10px; font-weight: bold;">장소</td>'
    + '<td style="padding: 10px;">' + (classData.location || '') + '</td></tr>'
    + '<tr><td style="padding: 10px; font-weight: bold;">강사</td>'
    + '<td style="padding: 10px;">' + (partner ? partner.partner_name : '') + '</td></tr>'
    + '</table>'
    + '<p style="color: #888; font-size: 13px;">* 준비물이 있다면 미리 확인해 주세요.</p>'
    + '</div>';
  ...
  if (partner && partner.email) {
    ...
    var partnerBody = '<div style="font-family: \'Noto Sans KR\', sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">'
      + '<h2 style="color: #2d2d2d;">수업 ' + daysLabel + ' 전 알림</h2>'
      + '<p>' + partner.partner_name + '님, ' + classData.class_name + ' 수업이 ' + daysLabel + ' 남았습니다.</p>'
      + '<p>일시: ' + (row.class_date || '') + '</p>'
      + '<p>예약 인원: ' + (row.student_count || 1) + '명</p>'
      + '</div>';
...
function sendReviewRequestEmail_(row) {
  ...
  // TODO: 수강생 이메일이 정산 내역에 없으므로,
  // 향후 student_email 컬럼 추가 또는 주문 조회 API로 이메일 획득 필요
  // 현재는 파트너에게만 후기 유도 안내

  if (partner && partner.email) {
    var subject = '[PRESSCO21] 수강생 후기 확인 안내 - ' + classData.class_name;
    var body = '<div style="font-family: \'Noto Sans KR\', sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">'
      + '<h2 style="color: #2d2d2d;">수강생 후기 확인</h2>'
      + '<p>' + partner.partner_name + '님, ' + classData.class_name + ' 수업에 대한 후기 작성 요청이 수강생에게 발송되었습니다.</p>'
      + '<p>파트너 대시보드에서 후기를 확인하고 답변해 주세요.</p>'
      + '</div>';

    sendEmailWithTracking_(partner.email, subject, body, 'REVIEW_REQUEST_PARTNER');
  }
...
function retryFailedSettlements() {
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    Logger.log('[재시도] settlement_id=' + row.settlement_id + ', partner=' + row.partner_code);
    ...
    var result = processReservePayment_(
      ...
      partner.member_id,
      reserveAmount,
      '[PRESSCO21] 적립금 재처리 - 주문: ' + row.order_id
    );
    ...
    // GAS 실행 시간 제한 대비 (5분 초과 시 중단)
    if (i % 10 === 0) {
      SpreadsheetApp.flush();
    }
  }

  Logger.log('[재시도 완료] 성공=' + retried + '건, 실패=' + failed + '건');

  // 결과를 관리자에게 알림
  if (retried > 0 || failed > 0) {
    sendAdminAlert_(
      '[PRESSCO21] FAILED 정산 재시도 결과',
      '재시도 완료\n성공: ' + retried + '건\n실패: ' + failed + '건\n\n'
      + '실패한 정산은 "정산 내역" 시트에서 확인하세요.'
    );
  }
}
